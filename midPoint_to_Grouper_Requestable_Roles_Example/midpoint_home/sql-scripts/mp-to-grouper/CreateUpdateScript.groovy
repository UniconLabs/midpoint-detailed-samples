...
Uid handleAccountMembershipUpdate(final Sql sql, final Uid uidValue) {
    ...
    if (subjectId?.trim() && updatesToMemberships?.size() > 0) {
        Set toInsert
        Set toDelete
        def currentMemberships

        if (OperationType.REMOVE_ATTRIBUTE_VALUES.equals(operation)) {
            toDelete = new HashSet<>(Arrays.asList(updatesToMemberships))
        } else {
            currentMemberships = getCurrentMemberships(sql, subjectId, log)?.get(subjectId)
            ...
            if (currentMemberships && !currentMemberships?.isEmpty()) {
                ...
                toInsert = new HashSet<>(Arrays.asList(updatesToMemberships))
            }
        }
